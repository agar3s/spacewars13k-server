...
class Singleton {
  ...
  private _owner: string = defaultValue<string>();
  ...
  private _decode(obj: JSON.Obj): Singleton {
    ...
    this._owner = obj.has("_owner") ? decode<string, JSON.Obj>(obj, "_owner"): defaultValue<string>();
...
@exportAs("new")
function init(owner: string): void {
  assert(isNull(__contract), "contract is already initialized");
  __contract = new Singleton(owner);
